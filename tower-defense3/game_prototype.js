...
class Tower {
  ...
  getSellValue(){
    // simple sell formula: 50% of (base + upgrades approximated)
    return Math.round(25 * this.level);
  }
  update(dt){
    this.cooldown -= dt;
    if(this.cooldown <= 0){
      ...
      if(target){ state.bullets.push(new Bullet(this.x,this.y,target,this.damage)); this.cooldown = this.fireRate; }
    }
  }
  draw(ctx){
...
function update(dt){
  // spawn logic: 1 enemy every 1.8s (simple)
  state.spawnTimer += dt;
...
canvas.addEventListener('click', (ev) => {
  ...
  const cost = 50;
  if(state.money >= cost){ state.towers.push(new Tower(x,y)); state.money -= cost; updateUI(); }
});
